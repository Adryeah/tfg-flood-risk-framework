...
const EN_ES = {
  ...
  "l'Horta Sud · 14 DANA-affected municipalities · Flood probability surface from Random Forest v3-T.":
    "l'Horta Sud · 14 municipios afectados por la DANA · Superficie de probabilidad de inundación del modelo Random Forest v3-T.",
  ...
  'Area under the ROC curve for the Random Forest v3-T model on the Valencia OOF set, averaged across 5 spatial GroupKFold folds (1×1 km blocks).':
    'Área bajo la curva ROC del modelo Random Forest v3-T sobre el conjunto OOF de Valencia, promediada sobre 5 folds espaciales GroupKFold (bloques de 1×1 km).',
  ...
  'Valencia OOF · threshold 0.310': 'Valencia OOF · umbral 0.310',
  ...
  'Harmonic mean of precision and recall at the operational decision threshold (0.310). Picked by maximising F1 on the validation folds — balances false alarms vs missed floods.':
    'Media armónica de precisión y recall en el umbral de decisión operativo (0.310). Elegido maximizando F1 en los folds de validación — equilibra falsas alarmas frente a inundaciones perdidas.',
  ...
  '2 × 2 contingency table of predictions at the operational threshold (0.310). TP = correctly flagged flood; FN = missed flood (worst case); FP = false alarm; TN = correctly clear pixel. Out-of-fold predictions only — no train-set leakage.':
    'Tabla de contingencia 2 × 2 de predicciones en el umbral operativo (0.310). TP = inundación marcada correctamente; FN = inundación no detectada (peor caso); FP = falsa alarma; TN = píxel seco correctamente clasificado. Solo predicciones out-of-fold — sin fuga del conjunto de entrenamiento.',
  ...
  'The transferability experiment demonstrates that model generalisation has identifiable, quantifiable and addressable limits. AUC 0.788 confirms ranking transfers; and with the v3-T model — after Leave-One-Zone-Out feature ablation removed the non-transferable spatial proxies (distance_to_coast, elevation) — the decision transfers too: extrapolated recall rises from 0 in v2 to 0.63 (0.92 with a 100 m buffer). Pixel-level precision stays low due to the rare event (0.3% prevalence), so the product aggregates risk to zone/policy level. Honestly delimiting the model validity domain (via Area of Applicability) is exactly the type of rigorous validation a thesis should include.':
    'El experimento de transferibilidad demuestra que la generalización del modelo tiene límites identificables, cuantificables y abordables. AUC 0.788 confirma que el ranking transfiere; y con el modelo v3-T — tras la ablación Leave-One-Zone-Out que elimina los spatial proxies no transferibles (distance_to_coast, elevation) — la decisión también transfiere: el recall extrapolado pasa de 0 en v2 a 0.63 (0.92 con buffer 100 m). La precisión a nivel píxel sigue baja por la rareza del evento (prevalencia 0.3%), por lo que el producto agrega el riesgo a nivel zona/póliza. Delimitar honestamente el dominio de validez del modelo (vía Area of Applicability) es exactamente el tipo de validación rigurosa que un TFG debe incluir.',
  ...
  '(original 0.310 was recalibrated because positive prevalence differs by ~27× — 0.29 % vs 7.98 %). Switch to Binary to apply the slider live to the map.':
    '(el original 0.310 se recalibró porque la prevalencia positiva difiere ~27× — 0.29 % vs 7.98 %). Cambia a Binario para aplicar el slider en vivo al mapa.',
