...
export function Topbar() {
  ...
  return (
    <header className="h-14 sticky top-0 z-[1100] flex items-center px-6 gap-6 border-b" style={{ backgroundColor: TOP_BG, borderBottomColor: 'rgba(0,0,0,0.18)', color: INK }}>
      <div className="flex items-center text-13 gap-2 min-w-0">
        <span style={{ color: INK_MUTED }}>Framework</span>
        {/* Project chip — small mono caps. Sits the TFG context next to
         *  the Framework wordmark on every page. */}
        <span
          className="text-10 font-mono uppercase tracking-[0.16em] px-1.5 py-0.5 rounded-sm"
          style={{
            color: INK_MUTED,
        ...
        <Icon name="chevron-right" size={14} style={{ color: INK_MUTED, opacity: 0.7 }} />
        <span className="font-medium tracking-tight truncate">{sectionTitle}</span>
        <Icon name="chevron-right" size={14} style={{ color: INK_MUTED, opacity: 0.6 }} />
        <span className="text-11 font-mono uppercase tracking-wider whitespace-nowrap" style={{ color: INK_MUTED }}>
          ENV · OPS · Valencia
        </span>

        {/* Live ingestion indicator — pulsing green dot + last S1 pass.
         *  Sits at the end of the breadcrumb so it's the only "moving"
         *  element in the topbar (the pulse draws the eye to it). */}
        <span
          className="ml-3 inline-flex items-center gap-1.5 text-11 font-mono tabular-nums px-2 py-0.5 rounded-sm whitespace-nowrap"
          style={{
            color: '#86EFAC',
      ...
      {/* Search button — opens GlobalSearch dialog. Real search now
       *  (client-side filter over policy IDs across all cached
       *  portfolios). Hot-key Ctrl/Cmd + K. */}
      <button
        onClick={() => setSearchOpen(true)}
        className="ml-auto inline-flex items-center gap-2 h-8 px-2.5 text-12 rounded transition-colors hover:bg-white/5"
        style={{
      ...
      <div className="flex items-center gap-1">
        ...
        <button className="w-8 h-8 flex items-center justify-center rounded transition-colors" style={{ color: INK_MUTED }}>
        ...
        <div className="h-6 w-px mx-2" style={{ backgroundColor: 'rgba(255,255,255,0.10)' }} />

        {/* Flat hairline initials — no gradient, no fill. Reads as a
         *  typographic mark, not a brand-colored avatar circle (which
         *  is the dashboard default `frontend-design` warns about). */}
        <div
          className="w-8 h-8 flex items-center justify-center cursor-pointer text-11 font-mono font-medium tracking-[0.15em]"
          style={{
