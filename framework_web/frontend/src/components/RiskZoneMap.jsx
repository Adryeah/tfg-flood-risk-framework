...
function OverlayPanel({
  ...
  // Colapsable. En md+ siempre abierto. En mobile arranca cerrado
  // (sólo header con count de capas activas) — el panel con 6 toggles
  ...
  return (
    ...
    >
       ...
       *  default. Chevron sólo se muestra en mobile. */}
      <button
        type="button"
        onClick={() => setOpenMobile((v) => !v)}
        className="w-full px-2.5 py-1.5 sm:px-3 sm:py-2 flex items-center justify-between gap-2 border-b border-border-default md:cursor-default md:pointer-events-none"
        aria-expanded={openMobile}
        aria-controls="overlay-panel-items"
      >
        <span className="inline-flex items-center gap-2 text-10 font-mono font-semibold text-text-tertiary uppercase tracking-wider">
          Overlays
          {/* Count badge — visible sólo en mobile cerrado, da feedback
           *  de cuántas capas hay activas sin tener que expandir. */}
          <span className="md:hidden inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-sm bg-bg-subtle text-text-secondary tabular-nums normal-case">
            {activeCount}
          </span>
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="md:hidden text-text-tertiary">
            <svg
              width="14"
              height="14"
              viewBox="0 0 24 24"
      ...
      <div
        id="overlay-panel-items"
        className={
          'px-2.5 py-1.5 sm:px-3 sm:py-2 space-y-1 sm:space-y-1.5 ' +
          // En mobile: si openMobile=false, ocultar. En md+ siempre visible.
